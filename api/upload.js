...
module.exports = async (req, res) => {
  ...
  // Ensures the file contents actually match the claimed type.
  if (!validateMagicBytes(buffer, mime_type)) {
    ...
    return res.status(400).json({
      error: 'File content does not match the declared type. Please upload a genuine PDF, JPG, or PNG.'
    });
  }

  // ── Build safe storage path ───────────────────────────────────
  const ext      = mime_type === 'image/jpeg' ? 'jpg'
                 : mime_type === 'image/png'  ? 'png' : 'pdf';
  ...
  if (uploadErr) {
    await writeAuditLog('upload_storage_error', payload.id, 'owner', null,
      { doc_type, error: uploadErr.message }, ip);
    return res.status(500).json({ error: 'Storage upload failed. Please try again.' });
  }

  // ── Remove old record for this doc type (re-upload flow) ──────
  ...
  if (oldDoc?.file_path) {
    // Delete old file from storage silently
  ...
  // ── Store file_path only (no permanent signed URL stored) ─────
  // Signed URLs are generated fresh on each admin/owner request.
  const { data: doc, error: dbErr } = await supabase
    ...
    .insert({
      ...
      file_path: safePath,
      file_url:  null,       // generated on-demand — never stored
      status:    'pending'
    })
    .select()
    .single();
  ...
  await writeAuditLog('doc_uploaded', payload.id, 'owner', doc.id,
    { doc_type, file_name: safeFileName }, ip);
